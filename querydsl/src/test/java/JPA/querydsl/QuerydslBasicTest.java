...
public class QuerydslBasicTest {
    ...
    public void group() throws Exception {
        List<Tuple> result = queryFactory
                ...
                .groupBy(team.name)
                //.having(item.price.gt(1000))  그룹화된 결과 제한
                .fetch();
    ...
    public void join_on_no_relation() throws Exception {
        ...
        List<Tuple> result = queryFactory
                .select(member, team)
                .from(member)
                .leftJoin(team).on(member.username.eq(team.name))  //member.team 으로 아이디 매칭 X
                .fetch();
    ...
    //기본 생성자로 DTO 만든 후 세터를 이용해서 값 주입
    //1.프로퍼티 접근 방법
    @Test
    ...
    //조건의 null 값에 따라서 쿼리가 동적으로 변하는 경우
